...
if(isset($_POST["submit"])){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT id, username, password, image, admin FROM users WHERE username = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $username, $hashed_password, $image, $admin);
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
							...
							if(!empty($_POST["remember"])){
								setcookie("c_user", $hashed_password, time() + (30 * 86400));
							}
...
<body>
	...
	<div class="main_conteiner" id="cont_margin"> <!--Main Conteiner of the page -->
	  ...
	  <?php
	  if(isset($_SESSION["account_created"]) === true  ){
	  ...
	  <h2>Login Form</h2>
        <form style="margin: 0 40% 0 40%;" method = "post" action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
			<div class="form-group">
			  ...
			  <input type="submit" class="btn btn-primary" value="Login" id="submit_btn" name="submit">
		</form>
		<p>If you don't have an account <a href="Register.php">Register</a></p>
		<p>If you forget your password <a href="Reset-Password.php">Resset Password</a></p>
      </div>
