...
require_once "config.php";
	...
	$cnt = 0;
	$res_name = $iLvl = $lvl = $res_slot = $res_type = $img = $res_quality = $res_stam = $res_str = $res_agi = $res_intel = $res_crit = $res_has = $res_mas = $res_ves = $res_sp = $res_eq = $res_armor = $res_desc = $res_bind = [];
	
	//if there are values from navbar searchbox
/* 	 if (!(empty($_GET['search']))){
		 $name = $_GET['search'];
		 $query = "SELECT *FROM items WHERE name LIKE '%{$name} %'";
	 } */
	
	if(isset($_POST["apply"]) or isset($_POST["nav_apply"]) or isset($_POST["nav_apply_type"]) or isset($_GET["search"])){//This trigger if the apply buttons are pressed.
		...
		if(!(empty($_POST['search']))){
			...
			if($slot != "NULL" && $quality != "NULL" && $type != "NULL"){ //This trigger if the slot, quality and type are selected.
				...
				$query = "SELECT items.name, slots.name as 'slot', items.ilvl, items.lvl, type.name as 'type' , items.image, quality.name as 'quality', items.stamina, items.strength, items.agility, 
					items.intellect, items.critical, items.haste, items.mastery, items.versatility, items.sellprice, items.equip, items.armor, items.description, binds.name as bind FROM items
					...
					(slots.name = '{$slot}') 
					AND (quality.name = '{$quality}') AND (type.name = '{$type}') AND items.category = '1'
					ORDER BY items.name;";
				...
				$query = "SELECT items.name, slots.name as 'slot', items.ilvl, items.lvl, type.name as 'type' , items.image, quality.name as 'quality', items.stamina, items.strength, items.agility, 
					...
					WHERE  (items.name LIKE '%{$name}%') AND 
					(slots.name = '{$slot}') 
					AND (quality.name = '{$quality}') AND items.category = '1'
					ORDER BY items.name;";
				...
				$query = "SELECT items.name, slots.name as 'slot', items.ilvl, items.lvl, type.name as 'type' , items.image, quality.name as 'quality', items.stamina, items.strength, items.agility, 
					...
					WHERE  (items.name LIKE '%{$name}%') AND 
					(slots.name = '{$slot}') 
					AND (type.name = '{$type}') AND items.category = '1'
					ORDER BY items.name;";
				...
				$query = "SELECT items.name, slots.name as 'slot', items.ilvl, items.lvl, type.name as 'type' , items.image, quality.name as 'quality', items.stamina, items.strength, items.agility, 
					...
					OR (quality.name = '{$quality}') AND items.category = '1'
					ORDER BY items.name;";
			}
		...
		if($result){//Take the result of the query and association in arrays.
			while($query_executed = mysqli_fetch_assoc($result)){
				...
				$res_armor[$cnt]  = $query_executed["armor"];
...
<body>
       ...
       <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			...
			<div class="form-inline">
				...
				<select name="type" class="custom-select mb-2 mr-sm-4"  >
					...
					<option <?php if($type == "Cloth"){ echo "selected ";} ?> value="Cloth">Cloth</option>
					<option <?php if($type == "Leather"){ echo "selected ";} ?> value="Leather">Leather</option>
					<option <?php if($type == "Mail"){ echo "selected ";} ?> value="Mail">Mail</option>
					<option <?php if($type == "Plate"){ echo "selected ";} ?> value="Plate">Plate</option>
					<option <?php if($type == "Jewelry"){ echo "selected ";} ?> value="Jewelry">Jewelry</option>
			...
			<div class="form-inline">
				...
				<select name="slot" class="custom-select mb-2 mr-sm-4" >
					...
					<option <?php if($slot == "Head"){ echo "selected ";}?> value="Head">Head</option>
					<option <?php if($slot == "Chest"){ echo "selected ";}?> value="Chest">Chest</option>
					<option <?php if($slot == "Shoulders"){ echo "selected ";}?> value="Shoulders">Shoulders</option>
					<option <?php if($slot == "Neck"){ echo "selected ";}?> value="Neck">Neck</option>
					<option <?php if($slot == "Tabard"){ echo "selected ";}?> value="Tabard">Tabard</option>
					<option <?php if($slot == "Waist"){ echo "selected ";}?> value="Waist">Waist</option>
					<option <?php if($slot == "Wrist"){ echo "selected ";}?> value="Wrist">Wrist</option>
					<option <?php if($slot == "Legs"){ echo "selected ";}?> value="Legs">Legs</option>
					<option <?php if($slot == "Feet"){ echo "selected ";}?> value="Feet">Feet</option>
					<option <?php if($slot == "Ring"){ echo "selected ";}?> value="Ring">Ring</option>
					<option <?php if($slot == "Trinket"){ echo "selected ";}?> value="Trinket">Trinket</option>
				  </select>
			</div>
			<div class="form-inline">
    ...
    <div class='row'>
	  ...
	  <p class='mb-2 mr-sm-2 white-color' style='padding-left: 10px;'>$res_slot[$i]</p>
	  <p class='mb-2 mr-sm-2 white-color' style='padding-left: 10px;'>$res_armor[$i] Armor</p>
