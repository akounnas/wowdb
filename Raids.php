...
if($link === false){//Check if the connection are succesfull.
	...
	if(isset($_POST["nav_apply_type"])){
		$name = $_POST['nav_apply_type'];
		$onload = false;
		
		
		$query = "SELECT * FROM raids WHERE name = '{$name}'";
		...
		if($result){//Take the result of the query and association in arrays.
			while($query_executed = mysqli_fetch_assoc($result)){
				...
				$content = $query_executed["description"];
...
<body>
	...
	if($onload){
		...
		echo "<div class='container-fluid'>
			<div class='row'>
			<div class='col bg-dark'><p style='text-align: center; color: white; font-size: 30px;'>Raids</p></div>
			</div>
		</div>";
	  echo"<form action="; echo htmlspecialchars($_SERVER["PHP_SELF"]); echo " method = 'post'>
		<div class='container-fluid'>
			<div class='row'>
					<div class='col-sm-4 bg-dark'><input type='submit' class='dropdown-item' name='nav_apply_type' value='Crucible Of Storms' style=' border: none; padding: 0; background: none; cursor: pointer; text-align: center; color: white; font-size: 20px; '></div>
					<div class='col-sm-4  bg-dark'><input type='submit' class='dropdown-item' name='nav_apply_type' value='Battle Of Dazaralor' style='border: none; padding: 0; background: none; cursor: pointer;  text-align: center; color: white; font-size: 20px;'></div>
					<div class='col-sm-4  bg-dark'><input type='submit' class='dropdown-item' name='nav_apply_type' value='Uldir' style='border: none; padding: 0; background: none; cursor: pointer; color: white;  text-align: center; font-size: 20px;'></div>
			</div>
		</div>
		</form>";
	}
	...
	if(!$onload){
		echo "<div class='container-fluid'>
			<div class='row' >
					<div class='col-sm bg-dark' style='padding-bottom: 2%;'><img src='$image' style='width: 100%; padding-top :2%;'></div>
			</div>
		</div>
		<div class='container-fluid'>
			<div class='row'>
					<div class='col bg-dark'><p class='white-color' style='text-align: center; color: white; font-size: 30px;'>$name</p></div>
			</div>
		</div>
		<div class='container-fluid'>
			<div class='row'>
					<div class='col bg-dark'><p style = 'color: white;'>$content</p></div>
			</div>
		</div>";
